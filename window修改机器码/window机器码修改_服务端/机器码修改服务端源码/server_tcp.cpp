...
bool initWinSock()
{
	...
	{
		cout << "socket初始化失败,请检查环境！" << endl;
		MessageBoxA(NULL, "socket初始化失败,请检查环境！", "error", 0);
		return false;
	}
	cout << "初始化成功" << endl;
	return true;
...
bool createSocket(SOCKET& listenScok)
{
	...
	if (listenScok == INVALID_SOCKET)
	{
		...
		MessageBoxA(NULL, "socket创建失败,接口被占用，请检查！", "error", 0);
		return false;
	}
...
bool bindIPandPort(SOCKET& listenScok, const string ip, const unsigned short port)
{
	...
	{
		cout << "本地IP绑定失败！" << endl;
		MessageBoxA(NULL, "IP绑定失败请检查IP地址是否正确，接口是否被占用，操作系统是否开放接口！", "error", 0);
		return false;
	}
	cout << "绑定本地IP完毕" << ip.c_str() << port << endl;
	return true;
}
...
//服务器接收方式，发成功连接，接收数据，发送命令
std::string server_tcp(int digit)
{
	...
	if (initWinSock() == TRUE && createSocket(listenScok) == TRUE && bindIPandPort(listenScok, "192.168.2.190", 10268) == TRUE)
	{
		...
		waitClientConnect(listenScok, clientSock);//5客户端连接成功
		sendData(clientSock, "Server connection succeeded");//向客户端发送数据
		receiveData(clientSock, data);//接收客户端数据
		if (data.length() == 36) {
			if (censor_string(data) == 0) {
				if (data == "uNQ0JU4P0uLBmCvFXHIq7BzzGV4BT6Mn8m4j" || mysql(data) == 1) {
					sendData(clientSock, "succeed");
				}
			}
			else {				
